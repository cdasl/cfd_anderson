...
def ftcs(u): # forward time central space
    u[1:-1] = u[1:-1]-0.5*c*(u[2:]-u[:-2])
    return u[1:-1]
...
def macCormack(u):
    ...
    # up[1:-1] = u[1:-1] - c*(u[1:-1]-u[:-2])
    # u[1:-1] = u[1:-1] - 0.5*c*(u[2:]-u[1:-1]+up[1:-1]-up[:-2])
    return u[1:-1] 
...
solvers = [ftcs,macCormack]
name = ["ftcs", "macCormack"]
